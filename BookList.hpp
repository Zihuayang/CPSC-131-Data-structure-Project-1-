...
class BookList
{
  ...
  public:
   ...
   ~BookList();
    ...
    // Mutators
    BookList & insert( const Book & book, Position    position = Position::TOP );                   // add the book to the top (beginning) of the book list
    BookList & insert( const Book & book, std::size_t offsetFromTop            );                   // inserts before the existing book currently at that offset

    BookList & remove( const Book & book          );                                                // no change occurs if book not found
    BookList & remove( std::size_t  offsetFromTop );                                                // no change occurs if (zero-based) offsetFromTop >= size()

    BookList & moveToTop( const Book & book );
  ...
  private:
    // Helper functions
    bool        containersAreConsistent() const;
